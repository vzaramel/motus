...
function run() {
  ...
  assert.equal(normalizeU32(42), 42);
  ...
  assert.equal(req.queryRef, 0xFFFFFFF9);
  assert.equal(req.signature, 0xFFFFFFF5);
  ...
  console.log('runtime-wasm contract tests passed');
}
